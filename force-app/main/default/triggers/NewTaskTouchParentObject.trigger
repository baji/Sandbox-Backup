trigger NewTaskTouchParentObject on Task (after insert) {
    ...
    Map<Id,Lead> callEmailLeadsToUpdate = new Map<Id,Lead>(); // this list will be populated only when Type= 'Call' or 'Email'
    ...
    if(Boolean.valueOf(myCS1.value__c)){
        for (Task tsk:Trigger.new) 
        {
            ...
            System.debug(' Type of Task '+tsk.type);
            // By Mayur
            if(tsk.WhoId!=null && tsk.Type!=null && (tsk.type=='Call' || tsk.type=='Email'))
            //if(tsk.WhoId!=null &&  (tsk.Subject =='Call' || tsk.Subject =='Email'))
            {
                String whoId = tsk.WhoId;
                String whoIdPrefix = whoId.substring(0,3);
                ...
                if (whoIdPrefix.equalsIgnoreCase('00Q')) 
                {
                    ...
                    Lead lead = new Lead(Id=tsk.WhoId, Last_Activity_Custom__c=System.now());
    
                    callEmailLeadsToUpdate.put(tsk.WhoId, lead);
                }
            }
        }
        ...
        //By Mayur
        if (!callEmailLeadsToUpdate.isEmpty()) {
            List<Lead> leads = callEmailLeadsToUpdate.values();
            update leads;
        } //Mayur code ends
    }
}